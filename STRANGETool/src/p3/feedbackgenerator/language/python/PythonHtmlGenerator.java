...
public class PythonHtmlGenerator {
	public static void generateHtml(String codepath1, String codepath2,
			String assignmentname1, String assignmentname2,
			String templateHTMLPath, String outputHTMLPath, String homePath,
			int minimumMatchLength, String languageCode,
			ArrayList<ArrayList<String>> additionalKeywords) throws Exception {

		// for default strings
		generateRequiredDataForInformation(codepath1, codepath2,
				minimumMatchLength, languageCode, additionalKeywords);
		String[] defaultInfoStrings = generateInformationStrings(codepath1,
				codepath2, "origtablecontent", "", true,languageCode);

		String sumTable = HtmlTableStringGenerator.getTableContentForSummary(
				commentMessage, syntaxMessage, tokenString1, tokenString2, languageCode);
		String sumExplanation;
		if (languageCode.equals("en"))
			sumExplanation = HtmlTableStringGenerator
					.getExplanationContentForSummary();
		else
			sumExplanation = HtmlTableStringGenerator
					.getExplanationContentForSummaryId();
		...
		if (homePath != null && homePath.length() > 0) {
			homeTab = "<button class=\"tablinks\" onclick=\"window.open('"
					+ homePath + "', '_self');\">Return to home</button>";
		}

		// for formatted code
		generateRequiredDataForInformation(codepath1 + ".formatted", codepath2
				+ ".formatted", minimumMatchLength, languageCode,
				additionalKeywords);
		String[] formattedInfoStrings = generateInformationStrings(codepath1
				+ ".formatted", codepath2 + ".formatted", "nowstablecontent",
				"nows", true, languageCode);

		/*
		 * for formatted code without comments. The preprocessing is not
		 * required anymore as it uses the same data as formatted code with
		 * comments.
		 */
		String[] formattedInfoStringsWithoutComments = generateInformationStrings(
				codepath1 + ".formatted", codepath2 + ".formatted",
				"syontablecontent", "syon", false, languageCode);

		/*
		 * for token renaming, it is similar to formatted code without comments
		 * except that some tokens are renamed.
		 */
		...
		while ((line = fr.readLine()) != null) {
			...
			if (line.contains("@messageformattedcode")) {
				line = line.replace("@messageformattedcode",
						messageFormattedCode);
			}
			...
			if (line.contains("@nowstablecontent")) {
				line = line.replace("@nowstablecontent",
						formattedInfoStrings[2]);
			}
			if (line.contains("@nowsexplanation")) {
				line = line
						.replace("@nowsexplanation", formattedInfoStrings[3]);
			}

			// for formatted code without comments
			if (line.contains("@syoncode1")) {
				line = line.replace("@syoncode1",
						formattedInfoStringsWithoutComments[0]);
			}
			if (line.contains("@syoncode2")) {
				line = line.replace("@syoncode2",
						formattedInfoStringsWithoutComments[1]);
			}
			if (line.contains("@syontablecontent")) {
				line = line.replace("@syontablecontent",
						formattedInfoStringsWithoutComments[2]);
			}
			if (line.contains("@syonexplanation")) {
				line = line.replace("@syonexplanation",
						formattedInfoStringsWithoutComments[3]);
			}

			// for renamed token mode
			if (line.contains("@renacode1")) {
				line = line.replace("@renacode1",
						formattedAndRenamedInfoStringsWithoutComments[0]);
			}
			if (line.contains("@renacode2")) {
				line = line.replace("@renacode2",
						formattedAndRenamedInfoStringsWithoutComments[1]);
			}
			if (line.contains("@renatablecontent")) {
				line = line.replace("@renatablecontent",
						formattedAndRenamedInfoStringsWithoutComments[2]);
			}
			if (line.contains("@renaexplanation")) {
				line = line.replace("@renaexplanation",
						formattedAndRenamedInfoStringsWithoutComments[3]);
			}
	...
	private static void generateRequiredDataForInformation(String filepath1,
			String filepath2, int minimumMatchLength, String languageCode,
		...
		ArrayList<FeedbackToken> syntaxString1 = PythonFeedbackGenerator
				.generateSyntaxTokenString(filepath1);
		...
		ArrayList<FeedbackToken> syntaxString2 = PythonFeedbackGenerator
				.generateSyntaxTokenString(filepath2);
		...
		commentMessage = FeedbackMessageGenerator
				.generateCommentDisguiseMessages(tokenString1, tokenString2,
						languageCode);

		// preprocessing the syntax tokens
		PythonFeedbackGenerator.syntaxTokenStringPreprocessing(syntaxString1,
				additionalKeywords);
		PythonFeedbackGenerator.syntaxTokenStringPreprocessing(syntaxString2,
				additionalKeywords);

		// get syntax reordering messages
		syntaxMessage = FeedbackMessageGenerator
				.generateSyntaxReorderingMessages(syntaxString1, syntaxString2,
						tokenString1, tokenString2, filepath1, filepath2,
						minimumMatchLength, languageCode);
	}

	private static String[] generateInformationStrings(String filepath1,
			String filepath2, String tableId, String mode,
		...
		if (mode.equals("syon")) {
			code1 = readCode1WithoutComment(filepath1, syntaxMessage,
					commentString1, tableId);
			code2 = readCode2WithoutComment(filepath2, syntaxMessage,
					commentString2, tableId);
		} else {
		...
		String tableContent = HtmlTableStringGenerator
				.getTableContentForMatches(messages, tableId, humanLanguageId);

		// get natural language explanation content
		String explanationContent = getExplanationContent(syntaxMessage,
				commentMessage, mode);

		return new String[] { code1, code2, tableContent, explanationContent };
	}

	private static String[] generateInformationStringsTokenRenaming(
			String filepath1, String filepath2, String humanLanguageId) throws Exception {
		// get the html strings
		String code1, code2;
		...
		// get table contents
		String tableContent = HtmlTableStringGenerator
				.getTableContentForMatches(syntaxMessage, tableId, humanLanguageId);
	...
	/*
	 * THESE METHODS ARE DIFFERENT WITH JAVA'S METHODS WITH THE SAME NAME AS
	 * THEY DEAL WITH ONE CHARACTER COMMENT (#). They generate strings
	 * representing the code files with some tags embedded. It applies a little
	 * about deterministic finite automata.
	 */

	private static String readCode1(String filepath, String tableId,
			ArrayList<FeedbackToken> commentString) throws Exception {
		...
		while ((line = fr.readLine()) != null) {
			...
			for (int col = 0; col < line.length(); col++) {
				...
				if (commentMessageIndex < commentMessage.size()
						&& row == commentMessage.get(commentMessageIndex)
								.getStartRowCode1()
						&& col == commentMessage.get(commentMessageIndex)
								.getStartColCode1()) {
					// close previous tag if any
					if (closeTagRequired) {
						/*
						 * This mechanism to guarantee no whitespaces between
						 * syntax and comment are highlighted. Quite messy.
						 */
						...
						int indexOfLastNonWhitespace = code
								.lastIndexOf(lastChar);
						// put the closing tag after that last char
						code = code.substring(0, indexOfLastNonWhitespace + 1)
								+ "</a>"
					...
					// set the comment ID
					commentMessageId = commentMessage.get(commentMessageIndex)
							.getVisualId();
					// prepare the beginning tag
					String commentTagHeader = "<a class='commentsim' id='"
							+ commentMessageId + sourceId + "' href=\"#"
							+ commentMessageId + targetId
							+ "\" onclick=\"markSelected('" + commentMessageId
							+ "','" + tableId + "')\" >";
					...
					// add commentindex
					commentIndex++;
				} else if (commentIndex < commentString.size()
						&& row == commentString.get(commentIndex).getStartRow()
					...
					/*
					 * this only applicable for python actually as the comment
					 * can only be one character long
					 */
					if (row == commentString.get(commentIndex).getFinishRow()
							&& col == commentString.get(commentIndex)
									.getFinishCol() - 1) {
						...
						&& row == commentString.get(commentIndex)
								.getFinishRow()
						&& col == commentString.get(commentIndex)
								.getFinishCol() - 1) {
					...
					code += c;
						...
						&& row == syntaxMessage.get(syntaxMessageIndex)
								.getStartRowCode1()
						&& col == syntaxMessage.get(syntaxMessageIndex)
								.getStartColCode1()) {
					// set the syntax ID
					syntaxMessageId = syntaxMessage.get(syntaxMessageIndex)
							.getVisualId();
					// prepare the beginning tag
					String syntaxTagHeader = "<a class='syntaxsim' id='"
							+ syntaxMessageId + sourceId + "' href=\"#"
							+ syntaxMessageId + targetId
							+ "\" onclick=\"markSelected('" + syntaxMessageId
							+ "','" + tableId + "')\" >";
					// put the header tag along with current character
					...
					if (closeTagRequired == false) {
						/*
						 * if the beginning has not given any link open tag, add
						 * it
						 */
						String syntaxTagHeader = "<a class='syntaxsim' id='"
								+ syntaxMessageId + sourceId + syntaxCounter
								+ "' href=\"#" + syntaxMessageId + targetId
								+ "\" onclick=\"markSelected('"
								+ syntaxMessageId + "','" + tableId + "')\" >";
			...
			if (closeTagRequired == true) {
				/*
				 * This mechanism to guarantee no remaining whitespaces at the
				 * end of the line are highlighted.
				 */
				// get the trimmed version of the code
				String codeTemp = code.trim();
				// get last non-whitespace char
	...
	private static String readCode2(String filepath, String tableId,
			ArrayList<FeedbackToken> commentString) throws Exception {
		/*
		 * this function cannot be merged to readCode1 since all get start and
		 * finish pos methods are different.
		 */
		// for source and target id on code
		char sourceId = 'b';
		char targetId = 'a';
	...
	/*
	 * THESE METHODS ARE DIFFERENT WITH JAVA'S METHODS WITH THE SAME NAME. This
	 * is the same as the previous two except that these methods automatically
	 * filter the comments so that they are not embedded on the strings.
	 */
	private static String readCode1WithoutComment(String filepath,
			ArrayList<FeedbackMessage> syntaxMessage,
		...
		while ((line = fr.readLine()) != null) {
			/*
			 * if the line is by default empty, put it as it is since that line
			 * is a result of Python formatting.
			 */
			if (line.length() == 0) {
				/*
				 * only add if the code is not empty. This to prevent empty line
			...
			for (int col = 0; col < line.length(); col++) {
				...
				if (commentIndex < comments.size()
						&& row == comments.get(commentIndex).getStartRow()
						&& col == comments.get(commentIndex).getStartCol()) {
					// close previous tag if any
					if (closeTagRequired) {
						/*
						 * This mechanism to guarantee that per line, no
						 * whitespaces after the last matched syntax are
						...
						char lastChar = codeTemp.charAt(codeTemp.length() - 1);
						// get the position of the last non-whitespace char
						int indexOfLastNonWhitespace = rcode
								.lastIndexOf(lastChar);
						// put the closing tag after that last char
						rcode = rcode
								.substring(0, indexOfLastNonWhitespace + 1)
								+ "</a>"
				...
				} else if (commentIndex < comments.size()
						&& row == comments.get(commentIndex).getFinishRow()
						&& col == comments.get(commentIndex).getFinishCol() - 1) {
					// if the end is found, mark that it is not comment anymore
					isInComment = false;
					commentIndex++;
	...
	private static String readCode2WithoutComment(String filepath,
			ArrayList<FeedbackMessage> syntaxMessage,
			ArrayList<FeedbackToken> comments, String tableId) throws Exception {
		/*
		 * this function cannot be merged to readCode1 since all get start and
	...
	/*
	 * THESE METHODS ARE DIFFERENT WITH JAVA'S METHODS WITH THE SAME NAME. This
	 * is expanded from previous two in terms of renaming some specific tokens.
	 */
	private static String readCode1AndRenamed(String filepath,
			ArrayList<FeedbackMessage> syntaxMessage, String tableId)
			throws Exception {
		// for source and target id on code
		char sourceId = 'a';
		char targetId = 'b';
		...
		for (int i = 0; i < tokenString1.size(); i++) {
			...
			if (curt.getType().equals("COMMENT"))
				...
				if (curt.getContentForComparison().equals("number data type")
						|| curt.getContentForComparison().equals(
								"string data type")
		...
		while ((line = fr.readLine()) != null) {
			...
			for (int col = 0; col < line.length(); col++) {
				String c = HTMLSafeStringFormat(line.charAt(col));
				/*
				 * if skipcounter is not zero, this iteration should not put any
				 * chars BUT it still needs to check its state toward other
				 * components. Hence, c is assigned with an empty string.
				 */
				if (skipCounter > 0) {
					c = "";
					skipCounter--;
				...
				// if the position matches with comment message, start to mark
				if (commentIndex < commentString.size()
						&& row == commentString.get(commentIndex).getStartRow()
						&& col == commentString.get(commentIndex).getStartCol()) {
					// close previous tag if any
					if (closeTagRequired) {
					...
					commentIndex++;
						...
						&& col == syntaxMessage.get(syntaxMessageIndex)
								.getStartColCode1()) {
					// get content
					String curContent = getSyntaxContent(row, col,
							renamedTokens, syntaxIndex);
					if (curContent != null) {
						/*
						 * if it is one of the renamed tokens, add the col. The
						 * row is not changed as all syntax tokens start and end
						 * at the same row.
						 */
						skipCounter = (renamedTokens.get(syntaxIndex)
								.getContent().length() - 1);
					...
					if (isInComment) {
						...
						if (curContent != null) {
							/*
							 * if it is one of the renamed tokens, add the
							 * content, increase the col and syntaxIndex.
							 */
							rcode += curContent;
							skipCounter = (renamedTokens.get(syntaxIndex)
									.getContent().length() - 1);
	...
	private static String readCode2AndRenamed(String filepath,
			ArrayList<FeedbackMessage> syntaxMessage, String tableId)
			throws Exception {
		// for source and target id on code
		char sourceId = 'b';
	...
	private static String getSyntaxContent(int row, int col,
			ArrayList<FeedbackToken> renamedTokens, int syntaxIndex) {
		if (syntaxIndex < renamedTokens.size()
				&& row == renamedTokens.get(syntaxIndex).getStartRow()
				&& col == renamedTokens.get(syntaxIndex).getStartCol()) {
			String content = renamedTokens.get(syntaxIndex)
	...
	private static String getExplanationContent(
			ArrayList<FeedbackMessage> syntaxMessage,
		...
		for (FeedbackMessage m : syntaxMessage) {
			s = s + "<div class=\"explanationcontent\" id=\"" + m.getVisualId()
					+ "he\">";
