...
public class PythonFeedbackGenerator {
	...
	public static void syntaxTokenStringPreprocessing(
			ArrayList<FeedbackToken> syntaxTokenString,
			ArrayList<ArrayList<String>> additionalKeywords) {
		/*
		 * rename all Identifiers and StringLiterals as their respective type +
		 * merging all IntegerLiteral and FloatingPointLiteral as number
		 */
		for (int i = 0; i < syntaxTokenString.size(); i++) {
			// if it is the beginning of keywords
			int pos = AdditionalKeywordsManager.indexOf(i, syntaxTokenString,
					additionalKeywords);
			if (pos != -1) {
				...
				if (type.equals("NAME")) {
					...
					cur.setContentForComparison("string literal");
						...
						|| type.equals("FLOAT_NUMBER"))
					cur.setContentForComparison("number literal");
			}
		}
	}

	public static void mergeAdjacentCommentsAndWhitespacesOnTokenString(
		...
		while (i < tokenString.size()) {
			...
			if (cur.getType().equals("COMMENT")) {
				...
				if (nextCommentPos != -1) {
					for (int k = i + 1; k <= nextCommentPos; k++) {
						cur.setContent(cur.getContent()
								+ tokenString.get(k).getContent());
					}

					// set finish row and col of cur as the comments are merged
					cur.setFinishRow(tokenString.get(nextCommentPos)
							.getFinishRow());
					cur.setFinishCol(tokenString.get(nextCommentPos)
							.getFinishCol());
					...
					/*
					 * ask the iteration to recheck from merged token. This is
					 * different to Java adjacent comment merging.
					 */
					i--;
				}
				...
				if (i + 1 < tokenString.size()
						&& tokenString.get(i + 1).getType().equals("WS")) {
					// merge the content
					cur.setContent(cur.getContent()
							+ tokenString.get(i + 1).getContent());
					// set finish post
	...
	public static void mergeAdjacentCommentsAndWhitespacesOnTokenStringUnused(
			ArrayList<FeedbackToken> tokenString) {
		// merge all adjacent comments/whitespaces as one comment/whitespace
		...
		while (i < tokenString.size()) {
			...
			if (cur.getType().equals("COMMENT")) {
				/*
				 * check whether after this token, there is at least one comment
				 * token for correct merging. Avoiding merging single comment
				 * with multiple whitespaces.
				 */
				boolean isNextCommentTokenExist = false;

				/*
				 * if found, merge all next comment and whitespace tokens which
				 * positions on real code are adjacent to current comment
				 */
				while (j < tokenString.size()) {
					 ...
					 * are adjacent.
					 */
					if (tokenString.get(j).getStartRow() == tokenString.get(
							j - 1).getFinishRow()
							&& tokenString.get(j).getStartCol() == tokenString
									.get(j - 1).getFinishCol()) {
				...
				/*
				 * for whitespaces, it is similar as for comments except that in
				 * addition to general rules, it stops once it finds
				 * non-whitespace tokens (comment tokens).
				 */
				while (j < tokenString.size()) {
					// it should be merged to current token if their positions
					// are adjacent and that token is whitespace
					if (tokenString.get(j).getType().equals("WS")
							&& tokenString.get(j).getStartRow() == tokenString
									.get(j - 1).getFinishRow()
							&& tokenString.get(j).getStartCol() == tokenString
									.get(j - 1).getFinishCol()) {
						content += tokenString.get(j).getContent();
						j++;
	...
	public static ArrayList<FeedbackToken> generateCommentAndWhitespaceTokens(
			String filePath) {
		/*
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (commentPos != -1) {
					...
					if (whitespacecontent.length() > 0) {
						...
						commentWhitespaceTokens.add(new PythonFeedbackToken(
								whitespacecontent, "WS", curLine, col
										- whitespacecontent.length() + 1,
								curLine, col, null));
					}

					// add the comment
					commentWhitespaceTokens.add(new PythonFeedbackToken(line
							.substring(commentPos), "COMMENT", curLine,
					...
					if (isInTripleQuoteString == false) {
						whitespacecontent += "\n";
						commentWhitespaceTokens.add(new PythonFeedbackToken(
								whitespacecontent, "WS", curLine, Math.max(0,
										line.length() - 1), curLine + 1, 0,
								null));
					}
				}
	...
	public static ArrayList<FeedbackToken> generateSyntaxTokenString(
			String filePath) {
		...
		try {
			...
			for (int index = 0; index < tokens.size() - 1; index++) {
				...
				if (type.equals("93") || type.equals("94")
						|| type.equals("NEWLINE"))
					continue;
				result.add(new PythonFeedbackToken(token.getText(), type, token
						.getLine(), token.getCharPositionInLine(), token
						.getLine(), token.getCharPositionInLine()
						+ token.getText().length(), token));
			}
