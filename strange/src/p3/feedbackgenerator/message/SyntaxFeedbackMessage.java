...
public class SyntaxFeedbackMessage extends MultipleFeedbackMessage {
	...
	private String getDetailedSyntaxDifferenceMessageEn() {
		...
		for (int i = 0; i < mismatchSyntaxMessages.size(); i++) {
			ArrayList<StandardFeedbackMessage> messages = mismatchSyntaxMessages.get(i).getMismatchTokenMessages();
			StandardFeedbackMessage cur = messages.get(0);
			int occurrence = mismatchSyntaxMessages.get(i).getMismatchTokenMessages().size();
			if (cur.getAction().equals("modified")) {
				s += "\n\t<li> " + (cur.getDisguiseTarget().charAt(0) + "").toUpperCase()
						+ cur.getDisguiseTarget().substring(1) + " <span class=\"syntaxsim\">"
						+ JavaHtmlGenerator.HTMLSafeStringFormatWithSpace(cur.getToken1().getContent())
						+ "</span> is changed to <span class=\"syntaxsim\">"
						+ JavaHtmlGenerator.HTMLSafeStringFormatWithSpace(cur.getToken2().getContent())
						+ "</span> and used ";
				if (occurrence > 1)
					s += occurrence + " times.";
				else
		...
		s += "<table><tr class=\"inexplanation\">" + "<th class=\"inexplanation\">Aligned position</th>"
				+ "<th class=\"inexplanation\">Left form</th>" + "<th class=\"inexplanation\">Left position</th>"
				+ "<th class=\"inexplanation\">Right form</th>" + "<th class=\"inexplanation\">Right position</th>"
				+ "</tr>";
		for (int k = 0; k < tokenList1.size(); k++) {
			// get w1 info
			String w1 = tokenList1.get(k).getContent();
			/*
			 * if the comparison token is different to the raw content, add some info
			 * explaining the token for comparison.
			 */
			if (w1.equals(tokenList1.get(k).getContentForComparison()) == false)
				w1 += " (" + tokenList1.get(k).getContentForComparison() + ")";
			String pos1 = "line " + tokenList1.get(k).getStartRow() + " column " + tokenList1.get(k).getStartCol();
			...
			String pos2 = "line " + tokenList2.get(k).getStartRow() + " column " + tokenList2.get(k).getStartCol();
			...
			if (w1.equals(w2) == false) {
				s += "<td class=\"inexplanation\"><span class=\"syntaxsim\">" + w1 + "</span></td>";
			} else {
			...
			// write second token
			if (w1.equals(w2) == false) {
				s += "<td class=\"inexplanation\"><span class=\"syntaxsim\">" + w2 + "</span></td>";
			} else {
	...
	private String getDetailedSyntaxDifferenceMessageId() {
		...
		for (int i = 0; i < mismatchSyntaxMessages.size(); i++) {
			...
			if (cur.getAction().equals("modified")) {
				s += "\n\t<li> " + (cur.getDisguiseTarget().charAt(0) + "").toUpperCase()
						...
						+ JavaHtmlGenerator.HTMLSafeStringFormatWithSpace(cur.getToken2().getContent()) + "</span> ";
				if (occurrence > 1)
		...
		s += "<table><tr class=\"inexplanation\">" + "<th class=\"inexplanation\">Posisi perataan</th>"
				+ "<th class=\"inexplanation\">Bentuk kiri</th>" + "<th class=\"inexplanation\">Posisi kiri</th>"
				+ "<th class=\"inexplanation\">Bentuk kanan</th>" + "<th class=\"inexplanation\">Posisi kanan</th>"
				+ "</tr>";
		for (int k = 0; k < tokenList1.size(); k++) {
			// get w1 info
			String w1 = tokenList1.get(k).getContent();
