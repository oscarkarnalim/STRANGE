...
public class JavaHtmlGenerator {
	public static void generateHtml(String codepath1, String codepath2, String assignmentname1, String assignmentname2,
		...
		boolean isFormatted1 = JavaCodeFormatter.formatCode(codepath1);
		boolean isFormatted2 = JavaCodeFormatter.formatCode(codepath2);
		...
		if (homePath != null && homePath.length() > 0) {
			...
			String text = "Return to home";
			if (languageCode.equalsIgnoreCase("id"))
				text = "Kembali ke laman utama";

			homeTab = "<button class=\"tablinks\" onclick=\"window.open('" + homePath + "', '_self');\">" + text
					+ "</button>";
		}
	...
	public static void generateHtmlForCSTRANGE(String codepath1, String codepath2, String dirname1, String dirname2,
			String simTags, String templateHTMLPath, String outputHTMLPath, int minimumMatchLength, String languageCode,
			String jplagTag, ArrayList<ArrayList<String>> additionalKeywords, boolean isSyntacticSim) throws Exception {

		/*
		 * isSyntacticSim is used to determine which comparison mode will be used as the
		 * basis of similarity in this HTML
		 */
		...
		if (isSyntacticSim == false) {
			tokenString1 = JavaFeedbackGenerator.generaliseTokens(tokenString1);
			tokenString2 = JavaFeedbackGenerator.generaliseTokens(tokenString2);
		}
		 ...
		 * that some tokens are renamed.
		 */
		String[] formattedAndRenamedInfoStringsWithoutComments = generateInformationStringsTokenRenaming(codepath1,
				codepath2, languageCode);
	...
	private static void generateRequiredDataForInformation(String filepath1, String filepath2, int minimumMatchLength,
		...
		tokenString1 = JavaFeedbackGenerator.generateFeedbackTokenString(filepath1);
		
		JavaFeedbackGenerator.mergeAdjacentCommentsOnTokenString(tokenString1);

		tokenString2 = JavaFeedbackGenerator.generateFeedbackTokenString(filepath2);
		JavaFeedbackGenerator.mergeAdjacentCommentsOnTokenString(tokenString2);

		// get comment messages
		commentMessage = FeedbackMessageGenerator.generateCommentDisguiseMessages(tokenString1, tokenString2, "java",
				languageCode, isExplanationDetailed);
		


		// preprocess all content for comparison on these syntax token lists.
		ArrayList<FeedbackToken> syntaxString1 = JavaFeedbackGenerator.syntaxTokenStringPreprocessing(tokenString1,
				additionalKeywords, isSyntacticSim);
		ArrayList<FeedbackToken> syntaxString2 = JavaFeedbackGenerator.syntaxTokenStringPreprocessing(tokenString2,
				additionalKeywords, isSyntacticSim);
	...
	/*
	 * This is expanded from previous two in terms of renaming some specific tokens.
	 */
	private static String readCode1AndRenamed(String filepath, ArrayList<FeedbackMessage> syntaxMessage, String tableId)
		...
		for (int i = 0; i < tokenString1.size(); i++) {
			...
			if (curt.getType().equals("COMMENT"))
				...
				/*
				 * if it is not comment and whitespace, it is syntax token. check whether it is
				 * one of the renamed tokens.
				 */
